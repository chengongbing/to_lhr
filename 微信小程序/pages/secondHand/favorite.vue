		...
		<view class="wrap">
			...
			<swiper class="swiper-box" :current="swiperCurrent" @transition="transition"
							...
							<view class="order" v-for="(res, index) in orderList[0]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									...
									<view class="item" @click="detail(index,i)">
										...
										<view class="right">
											<view class="price">
												...
												<!-- <text class="decimal">.{{ priceDecimal(item.originalPrice) }}</text> -->
											</view>
											<!-- <view class="price">
											现价：￥{{ priceInt(item.price) }}
									...
									<view class="bottom">
										...
										<view class="logistics btn" @click="favorite(index,i)">取消收藏</view>
										<view class="exchange btn" @click="detail(index,i)" style="background-color: #ff7900;color: white;">购买</view>
							...
							<view class="order" v-for="(res, index) in orderList[1]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									<view class="top">
										...
										<view class="right" v-if="item.status==1">已售出</view>
										<view class="right" v-if="item.status==-1">已退货</view>
									</view>
									<view class="item" @click="detail(index,i)">
										<view class="left">
											<image :src="item.cover" mode="aspectFill"></image>
							...
							<view class="order" v-for="(res, index) in orderList[2]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									<view class="top">
										...
										<view class="right" v-if="item.status==1">付款成功</view>
										<view class="right" v-if="item.status==-1">退货成功</view>
									</view>
									<view class="item" @click="detail(index,i)">
										<view class="left">
											<image :src="item.cover" mode="aspectFill"></image>
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							<view class="order" v-for="(res, index) in orderList[5]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									<view class="top">
										<view class="left">
											<u-icon name="hourglass-half-fill" :size="30"
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							<view class="order" v-for="(res, index) in orderList[6]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									<view class="top">
										<view class="left">
											<u-icon name="hourglass-half-fill" :size="30"
				...
				<view>
					<scroll-view scroll-y="true" style="height: 1000rpx;">
						...
						<u-cell-group title="商品详情">
							...
							<view class="popup-group-view">
								数量：<text style="font-size: 34rpx; margin-right: 40rpx;">{{detailData.quantity}}</text>
							</view>
							<view class="popup-group-view">
								原价:<text
									style="color:#7d7d73 ;text-decoration: line-through;font-size: 34rpx; margin-right: 40rpx;">{{detailData.originalPrice}}</text>
								现价:<text style="color:red;font-size: 34rpx; ">{{detailData.price}}</text>
							</view>
						</u-cell-group>
						...
						<button style="width: 200rpx;margin-top: 20rpx;background-color: #ff7900;color: white;" @click="buy" v-if="detailData.status==0">购买</button>
						<button style="width: 300rpx;margin-top: 20rpx;background-color: #ff7900;color: white;" @click="view" v-if="detailData.status==1">查看联系方式</button>
	...
	export default {
		...
		methods: {
			favorite(index,i){
				...
				request.post("favorite/insert",this.fav).then(res => {
					if(res.code=='0')
					{
						uni.showToast({
							title:"收藏成功"
			...
			buy(){
				...
				request.post("secondHand/update",this.detailData).then(res => {
					if(res.code=='0'){
						uni.showToast({
							title:"购买成功"
						})
			...
			view(){
				request.post("student/phone",{"account":this.detailData.account}).then(res => {
					if(res.code=='0')
					{
			...
			search() {
				...
				request.post("secondHand/search", {
					"s": this.keyword,
			...
			getList(index) {
				...
				request.post("secondHand/classification", {
