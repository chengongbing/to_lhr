							...
							<view class="order" v-for="(res, index) in orderList[0]" :key="res.id">
								<view v-for="(item, i) in res.goodsList" :key="i">
									<view class="top">
										...
										<view class="right" v-if="item.auditStatus==-1">审核失败</view>
									</view>
	...
	export default {
		...
		methods: {
			...
			Delete(index, i) {
				...
				request.post("secondHand/delete", {
			...
			getMySale() {
				...
				request.post("secondHand/mySale", user).then(res => {
					// 判断返回数据的code值是否为0 
					if (res.code == '0') {
			...
			getSole() {
				const user = uni.getStorageSync("token")
				request.post("secondHand/sold", user).then(res => {
					// 判断返回数据的code值是否为0 
					if (res.code == '0') {
						// 将获取到的表单数据赋给form数组，并对每个表单进行处理 
						this.form = res.data
			...
			getPurchased() {
				const user = uni.getStorageSync("token")
				request.post("secondHand/purchased", user).then(res => {
					// 判断返回数据的code值是否为0 
					if (res.code == '0') {
						// 将获取到的表单数据赋给form数组，并对每个表单进行处理 
						this.form = res.data
