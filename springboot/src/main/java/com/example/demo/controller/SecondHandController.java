...
public class SecondHandController {
    ...
    public Result<?> findPage(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        LambdaQueryWrapper<SecondHand> wrapper = Wrappers.<SecondHand>lambdaQuery()
                .eq(SecondHand::getAuditStatus, 0)
                .like(SecondHand::getItemName , search);
        wrapper.like(SecondHand::getAuditStatus,0);
        if(StrUtil.isNotBlank(search))
        {
            wrapper.like(SecondHand::getItemName,search);
        };

        Page<SecondHand> assistancePage = secondHandMapper
